...
Issue.prototype = {
   ...
   ganttItem: function() {
      ...
      return {
         from: "/Date("+this.planning.start.valueOf()+")/",
         to: "/Date("+this.planning.end.valueOf()+")/",
         label: this.title,
...
Milestone.prototype = {
   ...
   plan: function() {
       ...
       this.minDate = moment(new Date(2018, 0, 1)).valueOf();
       this.maxDate = moment(new Date()).valueOf();
      ...
      this.issues.forEach(function(issue) {
         ...
         if(issue.planning.start.valueOf() < this.minDate)
            this.minDate = issue.planning.start.valueOf();

         if(issue.planning.end.valueOf() > this.maxDate)
            this.maxDate = issue.planning.end.valueOf();
   ...
   getGanttPlanning: function(options) {
      ...
      // null if no due date
       var releaseDate = !!this.due_on
	   ? moment(new Date(this.due_on)).startOf('day').valueOf() : null;
...
Developper.prototype = {
   ...
   plan: function(issue) {
      ...
      if(devHolidays) {
       ...
       while( config.weekDaysOff.indexOf(start.day()) != -1 ) {
           start.add(1, 'days');
       }

       var end = start.clone().add(durationInDays, 'days');

      // if [start,end] overlap week-ends, count the numbers of days to add
      if ( start.week() != end.week() ) {
          var diffWeeks = end.week()-start.week();
   ...
   getGanttPlanning: function() {
      ...
      if(config.holidays[this.name]) {
         var values = config.holidays[this.name].map(function(holiday) {
            return {
               from: "/Date("+holiday.start.valueOf()+")/",
               to: "/Date("+holiday.end.valueOf()+")/",
...
var Planning = {
    ...
    init: function (issues, milestones) {
